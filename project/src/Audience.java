...
public class Audience extends Person {
	...
	public boolean menu() {
        ...
        if(choice == 1) {
        	...
        	if(movieIndex <= 0 && movieIndex > movieSystem.numberOfMovies()) // film sayýsýný tutan bir deðiþken atanabilir.
        		System.out.println("There isn't any movie with this index!");
        	else {
        		selectedMovie = movieSystem.getMovie(movieIndex); // getMoviede iteratorle index kadar dolaþýlýp film döndürülebilir.
        	}
        }
        else if(choice == 2) {
	...
	private boolean movieMenu(Movie selectedMovie)
	{
    	...
    	if(choice == 1)
        	...
        	if(commentIndex <= 0 && commentIndex > selectedMovie.numberOfComments())) // comment sayýsýný tutan bir deðiþken atanabilir.
